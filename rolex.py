...
COOLDOWN_TIME = 90  # 90 seconds cooldown time
    ...
    if user_id in allowed_user_ids:
        ...
        if user_id not in admin_id:
            ...
            if user_id in bgmi_cooldown and (datetime.datetime.now() - bgmi_cooldown[user_id]).seconds < COOLDOWN_TIME:
                response = "ᴄᴏᴏʟᴅᴏᴡɴ ᴏɴ ᴘʟᴇᴀsᴇ ᴡᴀɪᴛ 1.5 ᴍɪɴᴜᴛᴇ ᴀɴᴅ ᴜsᴇ ᴀɢᴀɪɴ /bgmi ᴄᴏᴍᴍᴀɴᴅ❗ "
                bot.reply_to(message, response)
                return
        ...
        if len(command) == 4:  # Updated to accept target, port, and time
            ...
            if time > 90:
                response = "ᴇʀʀᴏʀ: ᴍᴀx ᴀᴛᴛᴀᴄᴋ sᴇᴄᴏɴᴅ 90sᴇᴄ ❌."
            else:
...
def show_help(message):
 ...
 /plan : ʙᴜʏ ғʀᴏᴍ ᴀᴅᴍɪɴ @Rolex_ddos.
...
def welcome_start(message):
    ...
    response = f"ᴍᴏsᴛ ᴡᴇʟᴄᴏᴍᴇ ɪɴ ʀᴏʟᴇx ᴘʀɪᴠᴀᴛᴇ ᴅᴅᴏs ᴜsᴇ ᴛʜɪs ᴄᴏᴍᴍᴀɴᴅ➡️: /help  \n @Rolex_ddos"
    bot.reply_to(message, response)
...
def welcome_plan(message):
    user_name = message.from_user.first_name
    response = f'''{user_name}, ʙᴜʏ ғʀᴏᴍ @Rolex_ddos
...
def restart_bot():
    while True:
        try:
            ...
            subprocess.run(['python3', os.path.join(current_directory, 'rolex.py')], check=True)
